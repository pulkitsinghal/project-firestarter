...
2. **Ship the contract the meta-layer expects.** The universal pieces assume:
   ...
   - A `.github/workflows/ci.yml` whose job **display names** are exactly
     ...
     `auto-merge.yml` gates on those names. Those names (and `Conventional
     Commits` / `Secret Scan`) must stay **globally unique** across all
     workflows: `auto-merge.yml` matches required checks by display name, so a
     collision in any *other* workflow shadows the real check.

...
| `.githooks/commit-msg` | Rejects automatic issue-closing directives in the complete message before enforcing the conventional-commit subject | both |
...
| `.github/workflows/commit-lint.yml` | Validates exact-base-range full commit messages, current PR title/body closure intent, and every conventional-commit subject; reruns on metadata edits | both |
| `.github/workflows/storyboard.yml` | Calls the stack-owned production-artifact `make storyboard` seam and uploads screenshots (non-blocking); path filters include the artifact-routing files | both (sibling origin) |
...
| `.github/dependabot.yml` + root `action-pin-catalog.yml` | Weekly `github-actions` updates keep immutable action SHAs current. The never-run root catalog makes every dormant template/stack/add-on action visible to Firestarter's Dependabot; a parity contract forces source propagation. Stack overlays may add their language ecosystems | domain-desk supply-chain posture, hardened for template-source upkeep (#116) |
...
| `docs/ROLLBACK.md` | Two-lane production recovery frame: evidence-gated application artifact promotion versus rare owner-gated database/state recovery, with disclosure and downstream effects reconciled separately; covers app/schema pairings, write fencing, recovery-point proof, DB-less state inventory, and asynchronous clients | generalized sibling rollback discipline (#1) |
...
| `docs/storyboard-harness.md` | What the storyboard/manifest harness is, why dev/stale output is not artifact evidence, and how to extend it | both |
...
| `docs/ENGINEERING_CONVENTIONS.md` | Nine reusable stack-neutral conventions the AGENTS brief points to: the authoritative quality gate (code review + the unit→integration→api→e2e pyramid, run locally on the exact candidate, sufficient quality evidence to merge but never a side-effect or merge-policy licence; absent/disabled/undispatched/billing-blocked/unavailable/zero-step hosted checks remain unexecuted—not green—and an executed failure cannot be overridden), stacked-PR merge order + recovery, forking discrete work into its own PR, asking for decisions visually rather than as a wall of text, bounded documentation-integrity preflight, immutable/updatable workflow dependencies, proof that Docker gates grade current—not stale—source, behavior-preserving canonical adoption through sanitized golden fixtures, byte parity, a pinned vendored source, one reversible shim, and a provenance/status table, plus an explicit portable/platform gate split that shares one target, documents the sole omission, composes evidence per dimension, and holds auto-merge until required platform proof exists | distilled cross-project engineering practice |
| `docs/PRACTICES.md` | Near-empty project memory for durable rules earned from failures: safe evidence → what it cost → generalized rule, plus a milestone sweep for lessons stranded in incident-local docs | sibling earned-practices log (#127) |
| `docs/postmortems/TEMPLATE.md` | Blameless postmortem template | sibling |
| `.github/pull_request_template.md` | Summary / Why / impact / test plan / evidence-backed handoff + checklist + exact final issue-closure block | sibling |
...
| `scripts/smoke.sh` | Syntax-checks the project's own shipped shell/hooks/python (`bash -n`/`sh -n`/`py_compile`; python3 optional, no host SDK), rejects remote GitHub Actions refs that are not full commit SHAs without echoing the target, and runs the dependency-light Docker-gate classifier regression suite. Wired into each stack's Tests job + `make smoke`/`precommit` | sibling script-smoke + domain-desk action pinning (#116) + blind-gate hardening (#107) |
| `scripts/gate-selftest.sh` + `scripts/tests/test-gate-selftest.sh` + `make gate-selftest` | Plants content-free source sentinels and invokes each stack's exact Docker runner definition. Unique per-run control/visibility markers distinguish current-source PASS, started-but-stale BLIND, and runner/build failure with UNKNOWN visibility; runner bytes stay private behind fixed categories, and sentinels/private logs clean up on exit/signals while cleanup failure overrides green without printing paths. Fake-runner plus semantic Compose/Dockerfile/ignore tests mutation-prove removed-mount, removed-build/context inclusion, dead/non-zero runner, marker-spoof, continuation, privacy, and cleanup behavior | four independently hardened sibling gate tripwires, generalized across every shipped stack (#107) |
| `scripts/check-docs.sh` + `make docs-check` | Dependency-free structural preflight: working mode checks tracked + non-ignored untracked Markdown; `--staged` checks an immutable index materialization. Requires house docs, calendar-valid VERSION/changelog alignment, resolves repository-local inline/reference and simple quoted HTML `href`/`src` paths with exact-case/publishable/symlink/escape checks, balances fences, and enforces allowlisted Mermaid types plus the sequence-diagram one-statement-per-line rule. Diagnostics suppress destinations; external URL, anchor, multiline HTML, full-parser, and render validation are explicitly not claimed | sample-notifier-service documentation tests, generalized and hardened (#114) |
| `scripts/check-closing-keywords.sh` | Content-suppressed guard for full commit messages, PR titles, and one exact final PR-body closure block; CI uses exact event SHAs and auto-merge revalidates fresh metadata from trusted default-branch code before emitting a fixed safe squash body | restricted sibling commit-numbering incident; identity/details withheld (#110) |
| `scripts/verify-release-parity.sh` | Fail-closed, content-suppressed deterministic-release guard for a trusted quiescent checkout: exact source→release byte pairs plus exact-set vendored SHA-256 manifests; exact on-disk case, retained single-open manifest descriptors, known-answer utility checks, and bounded temp cleanup under the physically resolved fixed system temp root reject parser/tool/path failures plus caller temp-path substitution and same/nested/hard-linked boundaries without disclosing paths, content, URLs, or hashes. Concurrent same-user manifest replacement is explicitly outside the portable Bash boundary; FIFO substitution can block, so keep the pipeline timeout, and route a hostile shared workspace through a separate sandbox/helper | sibling built-artifact release verification, generalized and hardened (#117) |
...
| `scripts/mutation-check.sh` + `scripts/mutation-cases.sh` + `make mutation-check` | Optional content-suppressed mutation proof: exact-one literal replacements, green focused baselines, assertion-specific private evidence plus typed exits, private byte snapshots, process-group deadlines and descendant cleanup, symlink/hardlink rejection, serialization, and interrupted-process recovery. Empty declarations truthfully SKIP and are never part of required gates | notebook-crypto mutation-proof loop, generalized and hardened (#115) |
...
| File | Purpose |
|------|---------|
| `docker-compose.yml` | postgres (pgvector) + redis + backend + HMR-friendly development frontend; `tools`/`node`/`storyboard` profiles. Storyboard targets a separate health-checked `frontend-storyboard` built from the Dockerfile's production stage. The `backend-tools`/`frontend-tools` services **mount live source** over the baked image so test/lint/seed run against the working tree, not stale baked source |
| `Makefile` | `up/down/migrate/test/lint/precommit/storyboard/hook-install` + `frontend-lockcheck`; `storyboard` starts dependencies, migrates, rebuilds the production frontend target, then captures it; `gate-selftest` proves both bind-mounted tool runners see current source before required gates — all via Docker |
...
| `frontend/` | Next.js App Router skeleton with an `/api` proxy to the backend; exact patched dependencies are locked and installed with `npm ci`; Dockerfile keeps separate `development` (`next dev`) and `production` (`next build` → `next start`) targets |
| `storyboard/` | Playwright runner pinned to `mcr.microsoft.com/playwright`, routed only to the health-checked production frontend target |
...
| File | Purpose |
|------|---------|
| `docker-compose.yml` | postgis (ARM64-safe `imresamu/postgis`) + redis + postgrest + gotrue; `dart`/`flutter`/`splash`/`storyboard` profiles. A dedicated health-checked `splash-storyboard` builds `dist/` and serves it through Vite preview; the Playwright service depends on that production-style path rather than a build-only/dev service |
| `Makefile` | Adds `flutter-analyze`, `flutter-format-check`, `dart-test`, `splash-build`, `pgrst-reload`; `storyboard` rebuilds the compiled splash serving path; `gate-selftest` proves all three bind-mounted tool runners see current source |
...
| `scripts/cloud-{snapshot,migrate,restore}.sh` + `scripts/cloud-db-lib.sh` + `scripts/check-cloud-migration-sql.py` | Opt-in Docker-only cloud recovery surface: true no-op defaults, non-overwriting verified role-neutral app-schema snapshots in private HTTPS S3-compatible storage, mandatory pre-snapshot incremental cloud migration, and exact-snapshot replay into an attested empty isolated drill target. A detached owner signature is verified against a committed public key; sanitized local replay intent, authenticated `verify-full` database TLS, same-connection stable-identity checks, an exported dump snapshot, staged migration bytes, a networkless transaction-escape guard, in-flight byte caps, checksums, and digest-pinned tools fail closed. Restore is recorded pending until project-specific application/data/security verification; there is no snapshot bypass, `latest`, archive grant/extension replay, credential value in argv/logs, or generic production restore. |
| `docs/CLOUD_DATABASE_RECOVERY.md` | Configuration and operation runbook, coverage exclusions, one-use authorization flow, snapshot/migration/drill lifecycle, indeterminate-outcome handling, and the boundary that keeps live restore project-specific under `docs/ROLLBACK.md`. |
...
| `splash/` | Minimal Vite + React + TS landing page with a committed npm lock; Node 22 + `npm ci` build deterministically, and storyboard serves freshly compiled `dist/` through the preview command with readiness |
| `storyboard/` | Playwright runner + manifest renderer pointed at the dedicated compiled-splash service |
...
- **Cloud migration is distinct from local migration:** it incrementally applies the same ordered files to a direct/session TLS target only after a verified immutable snapshot. Missing/indeterminate backup proof blocks; no generic bypass exists.
- **A restore drill is not production authority:** only an exact ready snapshot may restore into an attested empty isolated target. Live writer fencing, accepted write loss, dependency recovery, and application/external-effect verification stay in the project-specific rollback runbook.
...
| `Makefile` | `install/build/typecheck/test-unit/test/lint/precommit/clean/storyboard/e2e/hook-install`; storyboard and E2E preparation clean then rebuild exact `extension/dist` bytes; `gate-selftest` proves the bind-mounted tools runner sees current source; `up` builds + prints load-unpacked instructions, `down` cleans, `migrate` is a documented no-op |
...
| `e2e/` | Playwright MV3 harness: persistent headed Chromium loads the freshly rebuilt `extension/dist`, performs a direct side-panel smoke, and checks the real compiled content script against a local hostile fixture (duplicate/ambiguous/hidden/unsafe cases). Its static server is fixture input, not an app renderer. Xvfb makes it opt-in CI-capable; real toolbar attachment remains a distinct human release gate |
...
| `storyboard/` | After clean rebuild, fails if `dist/sidebar.html` is absent and screenshots that exact packaged side panel → committed `docs/STORYBOARD.md` (honours the storyboard precept for a UI with no server) |
...
| File | Purpose |
|------|---------|
| `docker-compose.yml` | Hardened API and worker containers built from the Dockerfile's production `runtime` target plus network-private Redis 8; `test` and `storyboard` profiles. Storyboard depends on the health-checked runtime API and worker. Only the loopback web port is published. |
| `Makefile` | Docker-only lifecycle, unit/integration gates, production image build, storyboard, cleanup, hooks, environment check, and version sync. `storyboard` rebuilds runtime API/worker before capture; `gate-selftest` proves the build-backed test runner includes current source; ordinary `down` preserves task state and `clean-data` is the explicit destructive path. |
...
| `storyboard/` | Drives the rebuilt production-runtime API/worker path, captures ready → in-flight → completed → acknowledged, and regenerates the planned-vs-implemented map. |
...
| `convergent_deploy` | `include_convergent_deploy` (default `no`) | **Stack-agnostic** (`addons/convergent_deploy/common/`): Python-stdlib additive/commutative manifest merge, live-first artifact healing and monotonic deploy fencing. Only a real 404 is absence; access interstitials, malformed JSON, timeouts and server errors fail closed. Optional runtime-only Cloudflare Access credentials authenticate reads and healing. Includes copy-ready unlisted-site headers/robots examples while the universal publishing doc keeps sensitive and source-specific material out | sibling static-share/review collision + access-gate incident |
...
Control bundle `1.4.6` and plugin `0.4.7` close the remaining real-ledger
...
| `bounded_runner` | `include_bounded_runner` (default `no`) | **Stack-agnostic** (`addons/bounded_runner/common/`): a Python-stdlib runner for trusted, finite argv units. Closed specs declare deadlines, worker weights and non-overlapping write paths; a fixed cross-run registry blocks colliding writers, POSIX process groups are terminated on deadline/heartbeat stall, exact fingerprints bind checkpoint resume, and closed-world aggregation makes every missing or unsuccessful required unit fail. Environment inheritance is allowlist-only, private state is size/mode checked, stale or malformed locks never auto-reap, and execution on Windows is routed through the Linux project container until equivalent process-tree proof exists. It does not assign tasks, supervise services, sandbox writes, use network APIs, or store credentials. | sibling research batch-run incidents (#124), implementation treated as untrusted reference |
...
| `encrypted_local_areas` | `include_encrypted_local_areas` (default `no`) | **Stack-agnostic** (`addons/encrypted_local_areas/common/`): the "encrypted local areas in a git repo" convention, so a stamped project can hold sensitive local data in-repo and still be pushed/backed up. git-crypt transparently encrypts a designated area (`{{ encrypted_paths }}/`, default `private`) at rest via a per-area `.gitattributes`. A **fail-closed** `scripts/git-crypt-guard.sh` checks staged blobs for git-crypt's magic header; `--status` separately proves a checkout unlocked only from a valid key in the current worktree's gitdir plus decrypted tracked content. The runbook warns that linked worktrees do not inherit unlock state and cleanup deletes their installed key copy, so encrypted repos prefer separate clones. Complements `secret_vault`; host-only, no new deps | sibling encrypted-area convention + linked-worktree key-loss incident |
...
| `agent_eval_harness` | `include_agent_eval_harness` (default `no`) | **Stack-agnostic** (`addons/agent_eval_harness/common/`): a Python-stdlib deterministic agent-evaluation harness. Versioned case/candidate/result schemas (`schemas/eval-*-1.0.schema.json`) locked in lockstep to closed `CheckKind`/`ResultState` enums; a pure reducer scores a candidate against blocking checks (`exact-equals`, `subset`, `regex-match`, `numeric-threshold`, `ordered-contains`) into one of `passed`/`failed`/`needs_review`/`unavailable`. Evidence is referenced by opaque id plus `sha256:` digest and verified, never inlined. An optional model critique is a structurally quarantined advisory field the scorer never reads, so it can never flip a blocking verdict. No dynamic imports, subprocess, or network. Synthetic fixtures with a coverage map; `run_validation` self-checks schema/enum/fixture parity. `docs/AGENT_EVAL_HARNESS.md` | clean-room derivation from Firestarter/Auggie eval needs plus neutral standards (JSON Schema 2020-12, NIST measurement concepts); no external submission consulted |
| `reviewed_extraction` | `include_reviewed_extraction` (default `no`) | **Stack-agnostic** (`addons/reviewed_extraction/common/`): a human-reviewed structured-extraction contract plus a deterministic egress reducer. A versioned claim carries `fieldPath`, proposed value, source id plus `sha256:` digest, evidence offsets, decision actor, purpose/recipient, and a closed state (`proposed`, `confirmed`, `corrected`, `unknown`, `omitted`, `conflicted`, `superseded`). The pure reducer folds claims per field with documented supersession/conflict/correction precedence; allowlisted egress emits only fields that are both requested and human `confirmed`/`corrected` (and purpose/recipient matched), so the model never authors the payload. Evidence offsets and digests are verified against caller-held source bytes that never enter the claim or egress artifacts. `docs/REVIEWED_EXTRACTION.md` | clean-room derivation from Firestarter/Auggie review needs plus neutral standards (W3C PROV, JSON Schema 2020-12, data-minimization/purpose-limitation); no external submission consulted |
| `mcp_security_gate` | `include_mcp_security_gate` (default `no`) | **Stack-agnostic** (`addons/mcp_security_gate/common/`): a deterministic offline MCP-server security gate. A closed risk-manifest schema describes tool schemas/effects, confirmation posture, untrusted-annotation handling, token audience/passthrough, output sanitization, secret/log leakage, timeouts, rate limits, size caps, and degraded-state behavior. A pure evaluator runs a closed 10-check catalog with stable reason codes; the overall disposition fails closed (any missing, unknown, or degraded posture is a FAIL, never a pass, and any one FAIL blocks). Fixed synthetic fake-MCP-server manifests exist only as static JSON fixtures (no process, socket, or network); the evaluator never imports or executes anything a manifest names. `docs/MCP_SECURITY_GATE.md` | clean-room derivation from Firestarter/Auggie needs plus neutral standards (the MCP specification's public concepts, NIST/OWASP control concepts, JSON Schema 2020-12); no external submission consulted |
| `architecture_manifest` | `include_architecture_manifest` (default `no`) | **Stack-agnostic** (`addons/architecture_manifest/common/`): a local closed architecture manifest with deterministic Mermaid/ANATOMY rendering and drift checking. The schema captures components, dependencies, data stores, trust boundaries, owner roles, and repo-relative evidence links (optional `sha256:` digests), all with opaque ids; absolute paths, traversal, hosts, and identity are rejected. Renderers are pure and order-independent (stable sorted ids, no timestamps); `check_drift` re-renders and compares by normalized-content digest, returning `current`/`stale`/`malformed` (fail-closed, no false `current`). No hosted graph, OAuth, source upload, AST crawler, or AI requirement. `docs/ARCHITECTURE_MANIFEST.md` | clean-room derivation from Firestarter/Auggie needs plus neutral standards (W3C PROV, JSON Schema 2020-12, general Mermaid conventions); no external submission consulted |
...
| `project_slug` | 1–40 character lowercase kebab id; drives db name, container prefix, package names, and a hyphen-to-underscore-safe migration table identifier |

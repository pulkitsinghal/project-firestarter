...
| `.github/workflows/deploy.yml` | `workflow_dispatch` one-button beta deploy on a **self-hosted** runner: `make up` + `make deploy` (Cloudflare quick-tunnel, no secrets). Universal because both stacks share those targets | sibling |
...
| `.github/pull_request_template.md` | Summary / Why / Migration impact / Test plan + checklist | sibling |

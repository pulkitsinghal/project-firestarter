...
| `datastore_advisor` | `include_datastore_advisor` (default `no`) | **Stack-agnostic** (`addons/datastore_advisor/common/`): makes "where should this project's data live?" an explicit decision instead of an inherited scaffold default. A stdlib-only elicitation tool (`tools/datastore_advisor/`) plus the durable guide `docs/DATASTORE_ADVISOR.md` and a committed decision record `docs/DATASTORE_DECISION.md`. Deliberately **not a quiz**: no weighted score, because a total hides the reason. It disqualifies with stated rules and echoes the answers that fired them, always emits a **case against its own recommendation**, names **what would change this answer**, and treats "unknown" as a measurement task rather than a silent default. Axis zero is *do you even want to hold this data* — device-local/client-encrypted removes a liability class rather than managing it. Two verifications are **unconditional**: (1) confirm the **edition/plan you will actually run** has the feature you are choosing the engine for — free/community editions routinely omit the isolation, RBAC and online-backup properties the engine was picked for, and the gap is silent until an audit (cited vendor example in the guide); (2) **restore a backup into a working system** at least once — a recommendation is not `operationally_ready` while that is outstanding. Catalog covers Postgres (+pgvector), SQLite/embedded, device-local, Neon, Supabase, Cloud SQL/RDS-class, DynamoDB, Firestore, Redis/Valkey and real graph DBs, each with a **`wrong_for` list** (the self-test fails any entry that only carries a pitch) and citations carrying URL + date read. Biased toward boring: for ordinary workloads it says "one Postgres instance" plainly. Advisory only — provisions and migrates nothing. No new tokens, no new deps | recurring pattern of scaffold-default datastores and engines chosen for properties their deployed edition lacks |

...
| `scripts/verify-release-parity.sh` | Fail-closed, content-suppressed deterministic-release guard: exact source→release byte pairs plus exact-set vendored SHA-256 manifests; exact on-disk case, single-open manifest descriptors, known-answer utility checks, and bounded temp cleanup under the physically resolved fixed system temp root reject parser/tool/race/path failures plus caller temp-path substitution and same/nested/hard-linked boundaries without disclosing paths, content, URLs, or hashes | sibling built-artifact release verification, generalized and hardened (#117) |

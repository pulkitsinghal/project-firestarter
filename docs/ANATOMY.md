...
| `Makefile` | `up/down/migrate/test/lint/precommit/storyboard/hook-install` + `frontend-lockcheck`; `gate-selftest` proves both bind-mounted tool runners see current source before required gates — all via Docker |
...
| `Makefile` | Adds `flutter-analyze`, `flutter-format-check`, `dart-test`, `splash-build`, `pgrst-reload`; `gate-selftest` proves all three bind-mounted tool runners see current source |
...
| `Makefile` | `install/build/typecheck/test-unit/test/lint/precommit/clean/storyboard/e2e/hook-install`; `gate-selftest` proves the bind-mounted tools runner sees current source; `up` builds + prints load-unpacked instructions, `down` cleans, `migrate` is a documented no-op |
...
| `Makefile` | Docker-only lifecycle, unit/integration gates, production image build, storyboard, cleanup, hooks, environment check, and version sync. `gate-selftest` proves the build-backed test runner includes current source; ordinary `down` preserves task state and `clean-data` is the explicit destructive path. |

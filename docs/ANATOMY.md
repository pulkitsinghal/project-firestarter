...
| `convergent_deploy` | `include_convergent_deploy` (default `no`) | **Stack-agnostic** (`addons/convergent_deploy/common/`): additive/commutative/fenced static-deploy helper plus a copy-ready `docs/examples/unlisted-site/{_headers,robots.txt}` pair. The universal publishing doc distinguishes non-sensitive unlisted links from authenticated review and keeps source-specific content, domains, identities, identifiers and credentials out of generated examples | sibling static-share/review sites |

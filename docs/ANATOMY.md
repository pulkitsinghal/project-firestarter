...
| `email_flywheel` | `include_email_flywheel` (default `no`) | **Stack-agnostic** (`addons/email_flywheel/common/`): an opt-in growth loop — the email sibling of `scheduled_agent`. `email-flywheel.yml` (dispatch by default, commented cron to run the recurring routine, least-privilege `contents:read`) + a dependency-free `scripts/email-flywheel.mjs` (Node 20 `fetch` + stdlib `node:net`/`node:tls`: lists merged PRs in the window → Anthropic Messages API drafts one short update email → dry-runs a preview to the run summary, or sends via the operator's **own** SMTP over verified TLS) + an editable `.github/agent/email-flywheel.prompt.md` + `docs/EMAIL_FLYWHEEL.md`. Safe by default: nothing sends until `SMTP_HOST` + `FLYWHEEL_RECIPIENTS` are set. Reuses `ANTHROPIC_API_KEY`; no new deps, no third-party mail subscription | growth loop |

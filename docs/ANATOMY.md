...
| `docs/ENGINEERING_CONVENTIONS.md` | Seven reusable stack-neutral conventions the AGENTS brief points to: the authoritative quality gate (code review + the unit→integration→api→e2e pyramid, run locally on the exact candidate, sufficient quality evidence to merge but never a side-effect or merge-policy licence; absent/disabled/undispatched/billing-blocked/unavailable/zero-step hosted checks remain unexecuted—not green—and an executed failure cannot be overridden), stacked-PR merge order + recovery, forking discrete work into its own PR, asking for decisions visually rather than as a wall of text, bounded documentation-integrity preflight, immutable/updatable workflow dependencies, and proof that Docker gates grade current—not stale—source | distilled cross-project engineering practice |

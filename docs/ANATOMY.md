...
| `cross_host_agent` | `include_cross_host_agent` (default `no`) | **Stack-agnostic** (`addons/cross_host_agent/common/`): lets an agent session on one machine do work that only exists on another, through one scoped channel rather than an improvised one. Python-stdlib. | one audited cross-host channel |
| `trusted_workstation` | `include_trusted_workstation` (default `no`) | **Stack-agnostic** (`addons/trusted_workstation/common/`): the read-only first phase of a trusted-workstation enrollment flow. Stamps a machine-readable policy; default-off and stack-neutral. | enrollment policy, read-only phase |
| `version_changelog` | `include_version_changelog` (default `no`) | **Stack-agnostic** (`addons/version_changelog/common/`): version stamp, changelog and rollback. User-facing doc is `common/docs/VERSION_CHANGELOG.md`. | releases you can roll back |

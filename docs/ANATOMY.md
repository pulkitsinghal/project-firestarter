...
| `docs/ENGINEERING_CONVENTIONS.md` | Five reusable stack-neutral conventions the AGENTS brief points to: the authoritative quality gate (code review + the unit→integration→api→e2e pyramid, run locally, sufficient to merge, never a side-effect licence), stacked-PR merge order + recovery, forking discrete work into its own PR, asking for decisions visually rather than as a wall of text, and bounded documentation-integrity preflight | distilled cross-project engineering practice |

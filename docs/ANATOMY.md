...
addons/                   OPTIONAL modules, overlaid only when opted in
  ...
  orchestrator_session/common/  Bill + schema-1.3 capacity/lifecycle/duration control plane + repo-local PM-proxy plugin (include_orchestrator_session=yes)
...
Control `1.3.6` adds one of those typed tools for capacity reconfiguration. It

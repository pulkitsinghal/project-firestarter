...
The harness must never gain a runtime that reaches the network, spawns a
process, or dynamically loads a caller-named module, and its synthetic fixtures
must never carry raw content (prompts, responses, hosts, identities).
"""
...
FORBIDDEN_IMPORTS = (
    r"^\s*(?:from|import)\s+(?:selenium|playwright|pyautogui|pynput|requests|socket|subprocess|importlib)\b",
    r"^\s*from\s+urllib\b",
)

# eval/exec of case content and dynamic validator loading are out of contract.
FORBIDDEN_CALLS = (
    r"\beval\s*\(",
    r"\bexec\s*\(",
    r"\b__import__\s*\(",
    ...
    for path in sorted(ROOT.rglob("*.py")):
        ...
        for pattern in FORBIDDEN_CALLS:
            if re.search(pattern, source):
                findings.append(
                    f"forbidden dynamic call in {path.relative_to(ROOT)}"
                )

    for fixture_path in sorted(FIXTURE_DIR.glob("*.json")):
        fixture = json.loads(fixture_path.read_text(encoding="utf-8"))
        if fixture.get("syntheticOnly") is not True:
            findings.append(f"fixture not marked syntheticOnly: {fixture_path.name}")

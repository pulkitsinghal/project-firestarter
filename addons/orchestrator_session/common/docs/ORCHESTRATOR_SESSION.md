...
| `orchestrator-control/` | Versioned generic policy ledger, schema-1.4 local SQLite authority, mandatory `root_role_guard.py`, evidence-derived lifecycle watchdog, duration-calibrated worker lanes, owner-operated single-leader federation, stable JSON CLI, schemas, security-hardened dashboard, and Phase-2 skill/plugin wrapper contract. |

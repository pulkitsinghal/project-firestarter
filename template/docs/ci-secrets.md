...
## gitleaks allowlist regexes are RE2 (no lookahead)

`secret-scan.yml` runs gitleaks, whose engine is Go **RE2**: no lookahead/
lookbehind, no backreferences. A pattern with `(?!…)` / `(?=…)` / `(?<…)` in
`.gitleaks.toml` **panics** the scanner and fails this required check — which
blocks auto-merge. When you allowlist a known public/test token, keep the regex
RE2-safe (character classes, anchors, and `\b` are fine; lookarounds are not).

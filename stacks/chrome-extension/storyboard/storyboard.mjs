// Storyboard harness (chrome-extension) — screenshots the built side panel and
// renders a planned-vs-implemented doc from manifest.json. The harness IS the
// canonical "what's implemented" source: regenerating docs/STORYBOARD.md every
// run keeps planned-vs-implemented from going stale. Run `make storyboard` (it
// builds first). Add a screen: build it, screenshot with shot('NN-name'), then
// reference the PNG in manifest.json.

import { chromium } from "playwright";
import fs from "node:fs";

const EXT_DIST = process.env.EXT_DIST ?? "/work/extension/dist";
...
function generateStoryboard() {
  ...
  const head = [
    ...
    "",
    "> **Auto-generated** by the screenshot harness (`storyboard/storyboard.mjs`)",
...
// Side panels are narrow — use a panel-ish viewport for the screenshots.
...
try {
  const sidebar = `${EXT_DIST}/sidebar.html`;
  if (!fs.existsSync(sidebar)) {
    throw new Error(`built side panel not found at ${sidebar} — run \`make build\` first`);
  }
  await page.goto(`file://${sidebar}`, { waitUntil: "networkidle" });
  await shot("01-sidebar");
  const heading = (await page.locator("h1").first().textContent().catch(() => "")) || "";
  assert("side panel renders the project name in <h1>", heading.includes("{{ project_name }}"));
} catch (e) {

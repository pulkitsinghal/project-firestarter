"""Generator and privacy contract for the mcp-security-gate add-on."""
...
class McpSecurityGateGeneratorTests(unittest.TestCase):
    def test_default_off_and_registered_as_closed_choice(self) -> None:
        config = json.loads((ROOT / "firestarter.config.json").read_text())
        self.assertEqual(config["include_mcp_security_gate"], ["no", "yes"])
        generator = GENERATOR.read_text()
        self.assertIn('"mcp_security_gate"', generator)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertFalse((output / "tools" / "mcp_security_gate").exists())
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for stack in config["stack"]:
                with self.subTest(stack=stack):
                    ...
                    generated = output / "tools" / "mcp_security_gate"
                    generated_files = {
            ...
            result = subprocess.run(
                [
                    ...
                    "-B",
                    "-m",
                    "tools.mcp_security_gate.run_validation",
                ],
            ...
            self.assertIn("mcp_security_gate: PASS", result.stdout)
    ...
    def test_version_matches_stamped_marker(self) -> None:
        version = (ADDON / "VERSION").read_text().strip()

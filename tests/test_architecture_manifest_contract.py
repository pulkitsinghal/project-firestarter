"""Generator and privacy contract for the architecture-manifest add-on."""
...
class ArchitectureManifestGeneratorTests(unittest.TestCase):
    def test_default_off_and_registered_as_closed_choice(self) -> None:
        config = json.loads((ROOT / "firestarter.config.json").read_text())
        self.assertEqual(config["include_architecture_manifest"], ["no", "yes"])
        generator = GENERATOR.read_text()
        self.assertIn('"architecture_manifest"', generator)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertFalse((output / "tools" / "architecture_manifest").exists())
        ...
        with tempfile.TemporaryDirectory() as temp:
                ...
                with self.subTest(stack=stack):
                    ...
                    subprocess.run(
                        [
                            ...
                            f"stack={stack}",
                            "--set",
                            "include_architecture_manifest=yes",
                            "--output",
                            str(output),
                        ],
                        cwd=ROOT,
                    ...
                    generated = output / "tools" / "architecture_manifest"
                    generated_files = {
                    ...
                    self.assertIn(Path("VERSION"), generated_files)
            ...
            result = subprocess.run(
                [
                    ...
                    "-B",
                    "-m",
                    "tools.architecture_manifest.run_validation",
                ],
            ...
            self.assertIn("architecture_manifest: PASS", result.stdout)

    def test_contracts_are_closed_and_runtime_has_no_capability(self) -> None:
    ...
    def test_version_is_present_and_pinned(self) -> None:
        self.assertEqual((ADDON / "VERSION").read_text().strip(), "0.1.0")

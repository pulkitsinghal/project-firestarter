"""Generator and privacy contract for the reviewed-extraction add-on."""
...
class ReviewedExtractionGeneratorTests(unittest.TestCase):
    def test_default_off_and_registered_as_closed_choice(self) -> None:
        config = json.loads((ROOT / "firestarter.config.json").read_text())
        self.assertEqual(config["include_reviewed_extraction"], ["no", "yes"])
        generator = GENERATOR.read_text()
        self.assertIn('"reviewed_extraction"', generator)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertFalse((output / "tools" / "reviewed_extraction").exists())
        ...
        with tempfile.TemporaryDirectory() as temp:
                ...
                with self.subTest(stack=stack):
                    ...
                    subprocess.run(
                        [
                            ...
                            f"stack={stack}",
                            "--set",
                            "include_reviewed_extraction=yes",
                            "--output",
                            str(output),
                        ],
                        cwd=ROOT,
                    ...
                    generated = output / "tools" / "reviewed_extraction"
                    generated_files = {
            ...
            result = subprocess.run(
                [
                    ...
                    "-B",
                    "-m",
                    "tools.reviewed_extraction.run_validation",
                ],
            ...
            self.assertIn("reviewed_extraction: PASS", result.stdout)

    def test_contracts_are_closed_and_runtime_has_no_egress_capability(self) -> None:
